...
const mybrandosConnector = connector({
  ...
  supportedOperations: ["mybrandos.inspectPublicDigitalLife", "mybrandos.listPublishedAssets", "mybrandos.createDraft", "mybrandos.publishDraft"],
  authenticationMode: "opaque-ref",
  capabilities: ["READ_ONLY", "PUBLIC", "REVERSIBLE_WRITE", "CONSEQUENTIAL_WRITE"],
  idempotencySupport: "REQUIRED",
  reconciliationSupport: "SUPPORTED",
  cancellationSupport: "UNSUPPORTED",
  healthState: "unknown",
...
const OPERATIONS: Record<string, DigiAiToolOperation> = {
  ...
  "mybrandos.publishDraft": {
    operationId: "mybrandos.publishDraft",
    connectorId: "mybrandos",
    operationName: "publishDraft",
    actionTypes: ["PUBLISH_MYBRANDOS_DRAFT"],
    inputSchema: schema(["draftId", "contentDigest"]),
    outputSchema: schema(["draftId", "state", "visibility", "ownerRef", "publishedAt", "approvedContentDigest", "publishedContentDigest", "source", "privacyTransition"]),
    riskClass: "high",
    sideEffectClass: "CONSEQUENTIAL_WRITE",
    idempotencyMode: "REQUIRED",
    reconciliationMode: "SUPPORTED",
