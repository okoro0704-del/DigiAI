...
import { signDraftSubject, signPublishSubject, type DraftSubjectAttestation, type PublishSubjectAttestation } from "./subject.js";
...
export type MybrandosPublishEvidence = {
  draftId: string;
...
export type MybrandosPublishResult =
  | { ok: true; status: number; body: MybrandosPublishEvidence; attempts: 1; submitted: true }
  | { ok: false; status: number; code: MybrandosReadFailureCode; attempts: 1; submitted: boolean };
...
function mapStatus(status: number, message = ""): MybrandosReadFailureCode {
  ...
  if (status === 409 && /approved_content_changed|content has changed/i.test(message)) return "APPROVED_CONTENT_CHANGED";
...
async function requestJson(input: {
  ...
  try {
    ...
    return { status: res.status, body: parsed, error: message || undefined };
  } catch (err) {
...
export async function requestMybrandosCreateDraft(input: {
  ...
  return { ok: false, status: last.status, code: mapStatus(last.status, last.error), attempts: 1, submitted: last.status >= 500 };
}
...
export async function reconcileMybrandosDraft(input: {
  ...
  return { ok: false, status: last.status, code: mapStatus(last.status, last.error), attempts: 1, submitted: false };
}

function parsePublishEvidence(raw: Record<string, unknown>, ownerId: string, draftId: string): MybrandosPublishEvidence {
  if (typeof raw.draftId !== "string" || raw.draftId !== draftId) {
    throw new DigiAiError(502, "MALFORMED_RESPONSE", "mybrandOS publish draft id is missing.");
  }
  if (raw.state !== "PUBLISHED" || raw.visibility !== "public" || raw.published !== true || raw.source !== "mybrandos") {
    throw new DigiAiError(502, "MALFORMED_RESPONSE", "mybrandOS did not return a public publication.");
  }
  if (typeof raw.ownerRef === "string" && raw.ownerRef !== ownerId) {
    throw new DigiAiError(502, "MALFORMED_RESPONSE", "mybrandOS returned a different owner.");
  }
  if (raw.privacyTransition !== "PRIVATE→PUBLIC" || raw.canonicalService !== "executePublish") {
    throw new DigiAiError(502, "MALFORMED_RESPONSE", "mybrandOS did not return governed publish evidence.");
  }
  return {
    draftId: raw.draftId,
    publicationRef: typeof raw.publicationRef === "string" ? raw.publicationRef : raw.draftId,
    state: "PUBLISHED",
    visibility: "public",
    ownerRef: ownerId,
    publishedAt: typeof raw.publishedAt === "string" ? raw.publishedAt : "",
    idempotencyKeyRef: typeof raw.idempotencyKeyRef === "string" ? raw.idempotencyKeyRef : "",
    approvedContentDigest: typeof raw.approvedContentDigest === "string" ? raw.approvedContentDigest : "",
    publishedContentDigest: typeof raw.publishedContentDigest === "string" ? raw.publishedContentDigest : "",
    publicPath: typeof raw.publicPath === "string" ? raw.publicPath : null,
    publicSlug: typeof raw.publicSlug === "string" ? raw.publicSlug : null,
    source: "mybrandos",
    privacyTransition: "PRIVATE→PUBLIC",
    published: true,
    scheduled: false,
...
export function attestPublishSubject(input: {
  secret: string;
  ownerId: string;
  draftId: string;
  idempotencyKey: string;
  ...
  ttlMs?: number;
}): PublishSubjectAttestation {
  return signPublishSubject(input.secret, {
    ownerId: input.ownerId,
...
export async function requestMybrandosPublishDraft(input: {
  config: MybrandosClientConfig;
  serviceSecret: string;
  ownerId: string;
  draftId: string;
  ...
  const host = registeredHost(input.config.baseUrl);
  const subjectContext = attestPublishSubject({
    secret: input.serviceSecret,
    ownerId: input.ownerId,
    draftId: input.draftId,
    idempotencyKey: input.idempotencyKey,
  ...
  const last = await requestJson({
    url: `${input.config.baseUrl.replace(/\/$/, "")}/api/internal/drafts/${encodeURIComponent(input.draftId)}/publish`,
    registered: host,
    timeoutMs: input.config.timeoutMs,
    serviceSecret: input.serviceSecret,
    method: "POST",
  ...
  if ((last.status === 200 || last.status === 201) && last.body) {
    try {
      return { ok: true, status: last.status, body: parsePublishEvidence(last.body, input.ownerId, input.draftId), attempts: 1, submitted: true };
    } catch {
      return { ok: false, status: 502, code: "MYBRANDOS_MALFORMED_RESPONSE", attempts: 1, submitted: true };
    }
  }
...
export async function reconcileMybrandosPublish(input: {
  config: MybrandosClientConfig;
  serviceSecret: string;
  ownerId: string;
  draftId: string;
  ...
  const last = await requestJson({
    url: `${input.config.baseUrl.replace(/\/$/, "")}/api/internal/drafts/${encodeURIComponent(input.draftId)}/publish/${encodeURIComponent(input.idempotencyKey)}`,
    registered: host,
    timeoutMs: input.config.timeoutMs,
    serviceSecret: input.serviceSecret,
    method: "GET",
  ...
  if (last.status === 200 && last.body) {
    try {
      return { ok: true, status: 200, body: parsePublishEvidence(last.body, input.ownerId, input.draftId), attempts: 1, submitted: true };
    } catch {
      return { ok: false, status: 502, code: "MYBRANDOS_MALFORMED_RESPONSE", attempts: 1, submitted: false };
    }
  }
